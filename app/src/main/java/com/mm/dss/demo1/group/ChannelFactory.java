package com.mm.dss.demo1.group;
...
public class ChannelFactory {
    ...
    private static class Instance {
        static ChannelFactory instance = new ChannelFactory();       //实例化ChannelFactory类
    }

    public static ChannelFactory getInstance() {
        return Instance.instance;       //获取实例化的ChannelFactory类
    }
    ...
    public void putChannelInfo(ChannelInfo info) {
        mChannelInfo.put(info.getChnSncode(), info);  //把通道数据写进mChannelInfo 键值对
    }

    public ChannelInfo getChannelInfo(String id) {
        return mChannelInfo.get(id);        //获取单个通道数据
    }

    public boolean isLoadChannelInfo(String id) {
        return mChannelInfo.containsKey(id);          //判断是否写进ChannelInfo
    }

    public List<ChannelInfo> getChannelInfoListById(List<String> channelIds) { //list
        List<ChannelInfo> channelInfos = new ArrayList<>();
        if(channelIds != null) {
            for(String id : channelIds) {
                if(mChannelInfo.containsKey(id))
                    channelInfos.add(mChannelInfo.get(id));
            }
        }
    ...
    public List<String> getAllValues() {
        ...
        synchronized (threadLock) {
            ...
            while (it.hasNext()) {
                ...
                values.addAll(elements);     //nodeList
            }
        }
        return values;
    }
